...
#import "ZXHomePageViewModel.h"

#import "ZXFindPutCell.h"
...
@interface ZXHomePageVC ()<UITableViewDelegate,UITableViewDataSource,iCarouselDataSource,iCarouselDelegate,ContentShowViewDelegate,TitleViewDelegate>

@property (nonatomic,strong) UITableView *tableView;
@property (nonatomic,strong)ZXHomePageViewModel *homeVM;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section ==1) {
        ...
        // 发现新奇
        ZXDiscoveryCell *cell = [tableView dequeueReusableCellWithIdentifier:@"DCell"];
        ...
        // FindPutCell (三个按钮)
        ZXFindPutCell *cell = [tableView dequeueReusableCellWithIdentifier:@"FCell"];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ZXLog(@"viewForHeaderInSection");
    if(section < self.homeVM.section-2) {
        ZXTitleView *moreView =[[ZXTitleView alloc] initWithTitle:[self.homeVM mainTitleForSection:section] hasMore:[self.homeVM hasMoreForSection:section]];
...
- (void)contentShowViewClick:(NSInteger)tag {
    ...
    ZXDestinationVC *vc = [[ZXDestinationVC alloc] initWithAlbumId:[self.homeVM albumIdForSection:section index:row] title:[self.homeVM mainTitleForSection:section]];
    // 隐藏状态栏及底部栏
...
- (void)titleViewDidClick:(NSInteger)tag {
    ...
    switch (tag) {
        case 0:
        {
            ZXEditorMoreVC *vc = [ZXEditorMoreVC new];
            // push 跳转控制器 隐藏tabbar
            vc.hidesBottomBarWhenPushed = YES;
        ...
        case 1:
        {
            ZXSpecialMoreVC *vc = [ZXSpecialMoreVC new];
            [self.navigationController pushViewController:vc animated:NO];
        }
            break;
        default:
        {
            // 如果>=2  跳转到中介页
            ZXJumpVC *vc = [[ZXJumpVC alloc] initWithCategoryId:_categoryId contentType:_type tag:tag];
            // push 跳转控制器 隐藏tabbar
            vc.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:vc animated:NO];
        }
...
#pragma mark - 懒加载
- (ZXHomePageViewModel *)homeVM {
     if (!_homeVM) {
         _homeVM = [ZXHomePageViewModel new];
     }
     return _homeVM;
 }
...
- (UITableView *)tableView{
    if (!_tableView) {
        ...
        [_tableView registerClass:[ZXFindPutCell class] forCellReuseIdentifier:@"FCell"];
        [_tableView registerClass:[ZXDiscoveryCell class] forCellReuseIdentifier:@"DCell"];
        ...
        //封装好的头部滚动视图
        ZXFocusImageScrollView *scrollView =[[ZXFocusImageScrollView alloc] initWithFocusImgNumber:self.homeVM.focusImgNumber];
