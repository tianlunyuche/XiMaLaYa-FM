...
// 除了热门推荐其他 ( 编推荐+发现新奇+精品听单+热门直播(entrances)+查看更多分类)
#define kOtherCount 5
// 热门推荐前面 (小编推荐+发现新奇+精品听单)
#define kFrontCount 3

@interface ZXHomePageViewModel ()

@property(nonatomic,strong)ZXHomePageModel *model;

@end

@implementation ZXHomePageViewModel

- (void)getDataCompletionHandle:(void (^)(NSError *))completed{
    self.dataTask =[ZXHomePageNetManager getHomePageIntroduceCompletionHandle:^(ZXHomePageModel *responseObject, NSError *error) {
...
- (NSURL *)coverURLForSection:(NSInteger)section index:(NSInteger)index{
    ...
    if (section == 0) {
        ...
        if (self.model.hotRecommends.list[section - 3].list.count >index) {
