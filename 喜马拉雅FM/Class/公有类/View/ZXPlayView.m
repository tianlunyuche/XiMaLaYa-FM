...
@implementation ZXPlayView

- (instancetype)init{
    
    if (self =[super init]) {
        //布局
        UIImageView *backgroundIV =[[UIImageView alloc ]initWithImage:[UIImage imageNamed:@"tabbar_np_normal"]];
        ...
        [backgroundIV addSubview:_circleIV];
...
- (CADisplayLink *)link{
    ...
    if (!_link) {
        ...
        //手动将定时器加到 事件循环中国年
        //NSRunLoopCommonModes 会使得RunLoop随着 界面切换仍继续使用，不然如果使用NSDefaultRunLoopMode的话UI交互没问题, 但滑动TableView就会出现不转问题, 因为RunLoop模式改变会影响定时器调度
