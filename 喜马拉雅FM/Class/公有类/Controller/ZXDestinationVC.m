...
// 头部展示页
#import "ZXAlbumHeaderView.h"
// 自定义Cell
#import "ZXMusicDetailCell.h"
#import "ZXTracksViewModel.h"

#import <UIKit+AFNetworking.h>

@interface ZXDestinationVC ()<UITableViewDataSource,UITableViewDelegate,AlbumHeaderViewDelegate>

@property (nonatomic,strong) UITableView *tableView;
@property (nonatomic,strong) ZXAlbumHeaderView  *infoView;
@property (nonatomic,strong) ZXTracksViewModel *tracksVM;
// 升序降序标签: 默认升序
@property (nonatomic,assign) BOOL isAsc;

@end

@implementation ZXDestinationVC
...
- (void)viewDidLoad {
    [super viewDidLoad];
    _infoView = [[ZXAlbumHeaderView alloc] initWithFrame:CGRectMake(0, -170, kWindowW, 170)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZXMusicDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ZXMusicDetailCell"];
...
- (ZXTracksViewModel *)tracksVM {
    if (!_tracksVM) {
        _tracksVM = [[ZXTracksViewModel alloc] initWithAlbumId:_albumId title:_oTitle isAsc:!_isAsc];
    }
    return _tracksVM;
}
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[ZXMusicDetailCell class] forCellReuseIdentifier:@"ZXMusicDetailCell"];
